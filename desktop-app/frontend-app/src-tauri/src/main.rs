...
fn terminate_sidecar(child: CommandChild) {
    #[cfg(windows)]
    {
        ...
        #[cfg(not(debug_assertions))]
        let _ = Command::new("taskkill")
            .args(["/IM", "fluxsql-sidecar.exe", "/T", "/F"])
            .output();
    }
    let _ = child.kill();
}
    ...
    let app = tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let mcp_bridge_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                ...
                .join("scripts")
                .join("fluxsql-mcp-stdio.mjs");

            #[cfg(not(debug_assertions))]
            let mcp_bridge_path = app
                .path()
                .resolve("fluxsql-mcp-stdio.mjs", BaseDirectory::Resource)?;
            ...
            #[cfg(not(debug_assertions))]
            let command = app.shell().sidecar("fluxsql-sidecar")?;
